...
public class ConfigController {
  ...
  public Form config(@RequestHeader String authorization,
    ...
    final Map<String, String> withUnit = fhirConfiguration.getWithUnit().getOrDefault(useSameType, new HashMap<>());
    final List<String> withRequired = fhirConfiguration.getWithRequired().getOrDefault(useSameType, new ArrayList<>());
    this.applyFormHpByTypeOrDefault(useSameType, form, lang, hp, hpByTypes);
    this.applyFormObservationByTypeOrDefault(useSameType, form, lang, observation, obsByTypes, multiValues, withUnit, withRequired);
  ...
  private void applyFormObservationByTypeOrDefault(String formType, Form form, String lang, CodeSystem all, List<ValueSet> byTypes, List<ValueSet> multiValues, Map<String, String> withUnit, List<String> withRequired) {
    ...
    if (byType.isPresent()) { // if this panel code has specific values or else use the default
      ...
      form.getConfig().getParaclinicalExams().getDefaultList().addAll(fhirToConfigMapper.mapToParaclinicalExams(all, lang, multiValues, withUnit, withRequired));
    }
    // sort by name
    final NormalizedComparator comparator = new NormalizedComparator();
